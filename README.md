...
| Part | Host |
|------|------|
| Backend (Flask) | [Render](https://render.com) free Web Service |
| Frontend (React) | [Netlify](https://www.netlify.com) or [Vercel](https://vercel.com) |

### 0) Push code to GitHub first

Render / Netlify / Vercel deploy from a GitHub repo.

1. Create a new GitHub repository  
2. Push this project (`backend/`, `frontend/`, `README.md`, etc.)  
3. Continue below  

### 1) Deploy backend on Render

1. Go to [https://render.com](https://render.com) → sign up / log in with GitHub  
2. **New → Web Service** → connect your Zealthy repo  
3. Settings:
...
| Key | Value |
|-----|--------|
| `JWT_SECRET_KEY` | any long random string (e.g. a password generator string) |

5. Create the service → wait for deploy  
6. Copy your API URL, e.g. `https://zealthy-api.onrender.com`  
7. Open `https://YOUR-API.onrender.com/api/health` — should return `{"status":"ok"}`

**Notes**
- Free Render apps **sleep** after idle time; the first request can take ~30–60 seconds  
- SQLite on free Render resets if the instance is fully replaced; the app **re-seeds** Mark, Lisa, and the doctor automatically when the DB is empty  

### 2) Deploy frontend on Netlify (easiest)

1. Go to [https://www.netlify.com](https://www.netlify.com) → sign up with GitHub  
...
| Key | Value |
|-----|--------|
| `VITE_API_URL` | your Render URL with **no trailing slash**, e.g. `https://zealthy-api.onrender.com` |

5. Deploy site → open the Netlify URL  

### Alternative: Frontend on Vercel

1. [https://vercel.com](https://vercel.com) → Import repo  
2. Root directory: `frontend`  
3. Framework: Vite  
4. Env var: `VITE_API_URL` = your Render API URL  
5. Deploy  

### 3) Smoke-test the live demo

1. Open the Netlify/Vercel URL  
2. Patient login: `mark@some-email-provider.net` / `Password123!`  
3. **Doctor login:** **`doctor@zealthy.com`** / **`Doctor123!`**  
4. If login fails on first try, wait ~1 minute (Render waking up) and retry  

### Deploy files included in this repo
